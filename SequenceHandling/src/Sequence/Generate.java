...
public class Generate {
	...
	public static void run(Hashtable<String,String> T){
		...
		String kmerFile = inFile+"."+kmer+".kmers";
		if(!IOTools.fileExists(kmerFile))
			Generate.calculateKmerDistribution(dir, inFile,kmerFile , kmer);
		Generate.generateSequenceFile(dir, inFile, kmerFile, outFile, kmer);
	...
	public static void generateSequenceFile(String dir, String seqFile, String kmerFile, String outFile, int kmers){
		...
		try{
			...
			for(int j = 0; j < A.size();j++){
				...
				EW.println(A.get(j).getName()+"_generated");
